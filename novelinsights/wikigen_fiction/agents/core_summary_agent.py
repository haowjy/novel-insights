...
from novelinsights.wikigen_fiction.prompts.models import CoreSummaryModel
...
class CoreSummaryAgent(BaseAgent):
    ...
    def __init__(self, llm: LLMWrapper):
        super().__init__(llm, response_model=CoreSummaryResponse)
    ...
    def generate(self, content:ReadChapterResponse, prev_json: dict = None) -> CoreSummaryResponse:
        ...
        """
        prompt = self.prompt(content, prev_json)
    ...
    def _mock_generate(self, response:str, content:ReadChapterResponse, prev_json: dict = None) -> CoreSummaryResponse:
        """Mock the generate method for testing purposes."""
        prompt = self.prompt(content, prev_json)
        super()._mock_generate(prompt, response)
        self.last_response = CoreSummaryResponse(full_response=response, core_summary_json=parse_json(response))
        return self.last_response
