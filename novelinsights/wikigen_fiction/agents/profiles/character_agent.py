...
from novelinsights.wikigen_fiction.models import CharacterModel
...
```json
{json.dumps(CharacterModel.to_json_schema())}
```
...
class CoreSummaryAgent(BaseAgent):
    ...
    def generate(self, content:ReadChapterResponse, prev_json: dict = None) -> CoreSummaryResponse:
        ...
        """
        resp_str = self.llm.generate(self.get_prompt(content, prev_json))
        self.last_response = CoreSummaryResponse(full_response=resp_str, core_summary_json=parse_json(resp_str))
        self.agent_history.append(self.last_response)
